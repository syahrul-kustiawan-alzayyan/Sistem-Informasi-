...
export const movePengajuanPembaharuan = createAsyncThunk(
  ...
  async ({ id, status }, { rejectWithValue }) => {
    try {
      const url =
        status === "approved"
          ? "http://localhost:3002/approveMajelisTaklim"
          : `http://localhost:3002/pengajuanpembaharuan/${id}`;
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(movePengajuanPembaharuan.fulfilled, (state, action) => {
        ...
        console.log(
          `Data with ID: ${id} moved to ${
            status === "approved" ? "approved list" : "deleted"
          }.`
        );
      })
...
export const { addPengajuanPembaharuan, removePengajuanPembaharuan } =
  PengajuanPembaharuanSlicer.actions;
