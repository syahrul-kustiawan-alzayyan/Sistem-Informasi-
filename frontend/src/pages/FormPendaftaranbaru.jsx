...
function FormPendaftaranBaru() {
  ...
  const getUserIdFromToken = () => {
    ...
    try {
      ...
      return decoded.id; // Pastikan properti `id` sesuai dengan isi payload JWT
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    Object.keys(formData).forEach((key) => {
      if (formData[key] instanceof File) {
        ...
        console.log(
          `Appending file: ${key}, size: ${formData[key].size} bytes`
        );
      } else {
        formDataToSend.append(key, formData[key] || "");
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:3002/pengajuanbaru",
        formDataToSend,
        {
          headers: {
        ...
        <Form onSubmit={handleSubmit}>
          ...
          <fieldset className="fieldset">
            ...
            <Form.Group controlId="ktpJamaah">
              <Form.Label>
                Fotocopy KTP Jamaah (Minimal 15 Orang) (PDF)
              </Form.Label>
              <Form.Control
                type="file"
                name="ktpJamaah"
            ...
            <Form.Group controlId="aktaYayasan">
              <Form.Label>
                Fotocopy Akta Yayasan dan SK Menkumham (Jika Penyelenggara
                Yayasan) (PDF)
              </Form.Label>
              <Form.Control
                type="file"
