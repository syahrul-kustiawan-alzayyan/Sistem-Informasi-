...
import { fetchPengajuanBaru, removePengajuanBaru } from '../redux/PengajuanBaruSlicer';
...
const PengajuanBaruCard = () => {
  ...
  const handleReject = async () => {
    try {
      await axios.delete(`http://localhost:3001/pengajuanbaru/${selectedPengajuanBaru._id}`, {
  ...
  const uniqueKecamatan = [...new Set(PengajuanBaru.map((item) => item.kecamatan))];
  const uniqueKelurahan = [...new Set(PengajuanBaru.map((item) => item.kelurahan))];
          ...
          {selectedPengajuanBaru && (
            <>
              <div className="detail-container">
                <p><strong>Tanggal:</strong> {new Date(selectedPengajuanBaru.tanggal).toLocaleDateString()}</p>
                <p><strong>Nama Pemohon:</strong> {selectedPengajuanBaru.namaPemohon}</p>
                <p><strong>Alamat Pemohon:</strong> {selectedPengajuanBaru.alamatPemohon}</p>
                <p><strong>No. HP Pemohon:</strong> {selectedPengajuanBaru.noHpPemohon}</p>
                <p><strong>Nama Majelis:</strong> {selectedPengajuanBaru.namaMajelis}</p>
                <p><strong>Alamat Majelis:</strong> {selectedPengajuanBaru.alamatMajelis}</p>
                <p><strong>Kecamatan:</strong> {selectedPengajuanBaru.kecamatan}</p>
                <p><strong>Kelurahan:</strong> {selectedPengajuanBaru.kelurahan}</p>
                <p><strong>Nama Pimpinan:</strong> {selectedPengajuanBaru.namaPimpinan}</p>
                <p><strong>No. HP Pimpinan:</strong> {selectedPengajuanBaru.noHpPimpinan}</p>
                <p><strong>Tahun Berdiri:</strong> {selectedPengajuanBaru.tahunBerdiri}</p>
                <p><strong>Jumlah Jamaah:</strong> {selectedPengajuanBaru.jumlahJamaah}</p>
                <p><strong>Penyelenggara:</strong> {selectedPengajuanBaru.penyelenggara}</p>
              </div>
                ...
                {[
                  ...
                  selectedPengajuanBaru[file.key] && (
                    <p key={file.key}>
                      ...
                      <a href={selectedPengajuanBaru[file.key]} target="_blank" rel="noopener noreferrer">
                        Lihat File
                      </a>
                    </p>
                  )
